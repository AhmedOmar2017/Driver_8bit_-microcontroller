...
void PinDir(uint8_t Base, uint8_t pin, uint8_t state){
    if(state == output){
        ...
        ((*(volatile uint8_t *)((Base|0x80))) &= ~(1<<pin));
...
void PortDir(uint8_t Base, uint8_t state){
     if(state == output){
        ...
        (*(volatile uint8_t *)((Base|0x80))) = 0x00;



    }else{

        (*(volatile uint8_t *)((Base|0x80))) = 0xFF;
...
uint8_t PinRead(uint8_t Base, uint8_t pin){
if ((*(volatile uint8_t *)(Base)) & (1<<pin)) return 1;
else return 0;
}
...
void LCD_Command(uint8_t value){
        ...
        PortWrite(0x08, (value & 0xf0));
        PinWrite(0x05, 0, low);
        PinWrite(0x05, 1, high);
